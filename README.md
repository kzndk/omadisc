...
4. The toolbar channel menu follows the active pane. In a server channel it actively scans the full sidebar and Discord's channel browser to list every accessible channel from that server, including channels outside your opted-in sidebar, collapsed categories, delayed/off-screen rows and linkless voice/stage rows. It derives the server directory directly, so Discord does not need to render a **Browse Channels** link first. Choose one to open it in the same pane. **Focus pane** temporarily expands a chat and **Back to grid** restores your layout.
