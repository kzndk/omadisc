...
4. Use **All channels** in the toolbar to jump to any saved channel, including one hidden by the current layout. **Back to grid** restores your layout. Use page scale 65% / 80% for dense layouts.
...
- Official `https://discord.com` pages in separate sandboxed Electron WebContentsViews and a sandboxed account BrowserWindow sharing the same session. Optional presentation-only CSS provides the Omarchy channel layout and colors; there is no injected Discord script, remote preload, patched client, self-bot, private API implementation, token extraction or iframe-header bypass.
